...
for i in {01..47}; do
  ...
  ogr2ogr -f GeoJSON $output_dir/$code.geojson $output_dir/$code/r2ka$code.shp -nln kokuseichosa
  rm -rf $output_dir/$code
  rm $output_dir/$code.zip

  if [ $i -eq "01" ]; then
    cp $output_dir/$code.geojson $output_dir/all.geojson
  else
    ogr2ogr -f GeoJSON -update -append $output_dir/all.geojson $output_dir/$code.geojson -nln kokuseichosa
  fi

  rm $output_dir/$code.geojson
